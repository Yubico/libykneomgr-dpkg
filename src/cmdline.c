			 ...
			 const char *additional_error)
{
  ...
  while (1)
	...
	{0, 0, 0, 0}
      ...
      c =
	getopt_long (argc, argv, "hVmwslaD:i:M:S:r:d", long_options,
		     &option_index);
	...
	case 'r':		/* Use only a matching card reader.  */
	  ...
	  if (update_arg ((void *) &(args_info->reader_arg),
			  &(args_info->reader_orig),
			  &(args_info->reader_given),
			  &(local_args_info.reader_given), optarg, 0, 0,
			  ARG_STRING, check_ambiguity, override, 0, 0,
			  "reader", 'r', additional_error))
	    goto failure;
